...
annytab.notifier = function () {
  ...
  function create(type, message) {
    ...
    if (type === 'success') {
      icon = '<i class="fas fa-check-circle fa-fw"></i>';
    ...
    box.insertAdjacentHTML('beforeend', '<div class="annytab-notifier-padding">' + '<div class="annytab-notifier-icon">' + icon + '</div>' + '<div class="annytab-notifier-message">' + message + '</div>' + '<div class="annytab-notifier-progress"></div>' + '<div class="annytab-notifier-close"></div >');
    +'</div>';
    container.appendChild(box); // Fade in the message box

    annytab.effects.fadeIn(box, options.fade_duration, 'block'); // Add events
  ...
  function close(box) {
    ...
    var close_button = box.querySelector('.annytab-notifier-close'); // Check if the box is closing
    ...
    annytab.effects.fadeOut(box, options.fade_duration); // Wait for the fade effect to finish
  ...
  function removeContainer() {
    ...
    if (container !== null) {
      ...
      var boxes = container.querySelectorAll('.annytab-notifier-box'); // Loop boxes
