...
annytab.imagepreview = function () {
  ...
  function imagepreview(opts) {
    ...
    this.options = {
      image_class: 'annytab-imagepreview',
      alt: 'Preview'
    };
    ...
    var inputs = document.querySelectorAll('[data-imagepreview-container]'); // Add events
  ...
  function addEvents(ip, inputs) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      window.onload = inputs[i].addEventListener('change', function (event) {
        ...
        event.preventDefault(); // Get a image preview container

        var preview_container = document.getElementById(this.getAttribute('data-imagepreview-container')); // Clear the container (fastest way)
