...
annytab.html.editor = function () {
  ...
  function editor(opts) {
    ...
    this.options = {
      button_container: 'htmlButtonPanel',
      textarea: 'txtContents',
      preview: 'txtPreview',
      button_class: 'annytab-form-button'
    };
    ...
    this.pw = document.getElementById(this.options.preview); // Add buttons
    ...
    html += '<div id="annytab-html-editor-unminify" class="' + this.options.button_class + '">&#43;</div>'; // Add html to the container (fastest way)

    this.container.insertAdjacentHTML('beforeend', html); // Add events
  ...
  editor.prototype.togglePreview = function (button) {
    // Get buttons
    var buttons = this.container.querySelectorAll('input'); // Check if the textarea is visible

    if (isVisible(this.ta) === true) {
      // Set preview window content
      this.pw.innerHTML = this.ta.value; // Set button text

      button.innerHTML = '<>'; // Disable buttons
      ...
      // Set button text
      button.innerHTML = '&#9738;'; // Enable buttons
  ...
  editor.prototype.unminify = function () {
    // Variables
    var step = 0;
    var content = ''; // Get all nodes

    var nodes = this.ta.value.replace(/<\//g, '~~~</').replace(/</g, '~~~<').replace(/>/g, '>~~~').split('~~~'); // Loop all nodes
  ...
  function addEvents(ec, container) {
    ...
    var mb = container.querySelector('#annytab-html-editor-minify'); // Add toggle preview click event
    ...
    }, false); // Add minify click event

    window.onload = mb.addEventListener('click', function (event) {
      // Prevent default click behavior
      event.preventDefault(); // Minify html

      ec.minify();
    }, false); // Add unminify click event

    window.onload = umb.addEventListener('click', function (event) {
      // Prevent default click behavior
      event.preventDefault(); // Unminify html
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      window.onload = buttons[i].addEventListener('click', function (event) {
        // Prevent default click behavior
        event.preventDefault(); // Add a tag
  ...
  function surroundSelectedText(el, before, after) {
    // Get selection
    var selection = getSelection(el); // Replace text

    el.value = el.value.slice(0, selection.start) + before + selection.text + after + el.value.slice(selection.end); // Set cursor
  ...
  function insertText(el, text) {
    // Get selection
    var selection = getSelection(el); // Insert text

    el.value = el.value.slice(0, selection.start) + text + el.value.slice(selection.end); // Set cursor
