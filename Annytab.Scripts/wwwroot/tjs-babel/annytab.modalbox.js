...
annytab.modalbox = function () {
  ...
  function modalbox(opts) {
    ...
    this.options = {
      selector: '.annytab-modalbox-popup',
    ...
    resetVariables(this); // Get all links that should have a modalbox
  ...
  function addLinkEvents(mb, links) {
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      window.onload = links[i].addEventListener('click', function (event) {
        ...
        event.preventDefault(); // Open modalbox
  ...
  function addContainerEvents(mb, callback, parameters) {
    ...
    window.onload = mb.close_button.addEventListener('click', function (event) {
      // Prevent default click behaviour
      event.preventDefault(); // Close the container

      mb.close();
    }, false); // Add a close event for click outside wrapper

    if (mb.options.close_click_outside === true) {
      window.onload = mb.container.addEventListener('click', function (event) {
        // Prevent default click behaviour
        event.preventDefault(); // Close the modalbox
    ...
    if (mb.cancel_button !== null) {
      window.onload = mb.cancel_button.addEventListener('click', function (event) {
        // Prevent default click behaviour
        event.preventDefault(); // Disable buttons

        mb.cancel_button.setAttribute('disabled', true);
        mb.ok_button.setAttribute('disabled', true); // Close the modalbox
    ...
    if (mb.ok_button !== null && callback !== null) {
      window.onload = mb.ok_button.addEventListener('click', function (event) {
        ...
        mb.cancel_button.setAttribute('disabled', true); // Call the callback function
  ...
  function openWithHtml(mb, html, position, callback, parameters) {
    ...
    mb.container.insertAdjacentHTML('beforeend', '<div class="annytab-modalbox-wrapper ' + position + '">' + '<div class="annytab-modalbox-padding">' + html + '</div></div>' + '<div class="annytab-modalbox-close"></div >');
    document.body.appendChild(mb.container); // Get references
    ...
    mb.ok_button = mb.container.querySelector('.annytab-modalbox-content-button-ok'); // Fade in the container

    annytab.effects.fadeIn(mb.container, mb.options.fade_duration, 'block'); // Add container events
  ...
  modalbox.prototype.open = function (input) {
    ...
    if (data.html !== null) {
      ...
      var html = '<div class="annytab-modalbox-content">' + '<div class="annytab-modalbox-content-title">' + data.title + '</div>' + '<div class="annytab-modalbox-content-message">' + data.message + '</div>' + '<div class="annytab-modalbox-content-buttonpanel">' + '<input type="button" class="annytab-modalbox-content-button-ok" value="' + data.ok_text + '" />' + '<input type="button" class="annytab-modalbox-content-button-cancel" value="' + data.cancel_text + '" />' + '</div></div>'; // Open a modalbox
  ...
  modalbox.prototype.close = function () {
    ...
    var box = this.container; // Fade out the container

    annytab.effects.fadeOut(box, this.options.fade_duration); // Remove the container
